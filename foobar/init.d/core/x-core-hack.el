...
(define-key prog-mode-map (kbd "M-(") (x-wrap-with "("))
...
(defun x-recentf-exclude-p (file)
...
(defun x-enable-flyspell ()
  "Enable command `flyspell-mode' if `x-flyspell' is not nil."
  (when (and x-flyspell (executable-find ispell-program-name))
    (flyspell-mode +1)))

(defun x-cleanup-maybe ()
  "Invoke `whitespace-cleanup' if `x-clean-whitespace-on-save' is not nil."
  (when x-clean-whitespace-on-save
    (whitespace-cleanup)))

(defun x-enable-whitespace ()
  "Enable `whitespace-mode' if `x-whitespace' is not nil."
  (when x-whitespace
    ;; keep the whitespace decent all the time (in this buffer)
    (add-hook 'before-save-hook 'x-cleanup-maybe nil t)
    (whitespace-mode +1)))

(add-hook 'text-mode-hook 'x-enable-flyspell)
(add-hook 'text-mode-hook 'x-enable-whitespace)
...
(defun yank-advised-indent-function (beg end)
  ...
  (if (<= (- end beg) x-yank-indent-threshold)
...
(advise-commands "indent" (yank yank-pop) after
  "If current mode is one of `x-yank-indent-modes',
indent yanked text (with prefix arg don't indent)."
  (if (and (not (ad-get-arg 0))
           (not (member major-mode x-indent-sensitive-modes))
           (or (derived-mode-p 'prog-mode)
               (member major-mode x-yank-indent-modes)))
...
(defun x-maybe-enable-undo-tree ()
  "Enable `undo-tree' if `x-undo-tree' is not nil."
  (when x-undo-tree
